...
1. **لا ترفع ملف `client_secret.json` أو `token.json` إلى GitHub** (مُستثناة في `.gitignore`)
2. عند أول تشغيل سيفتح المتصفح لتسجيل الدخول بحساب Google
3. بعد المصادقة الأولى تُحفظ بيانات الاعتماد في `token.json` (JSON، وليس pickle)
4. جدولة النشر تتطلب أن تكون القناة مؤهلة (لها تاريخ جيد)
5. الصور المصغرة المخصصة تتطلب التحقق من حساب القناة
6. تُكتب سجلات التشغيل في `youtube_upload.log`
